...
def _extract_notes(body: str) -> str:
    ...
    pattern = re.compile(
        r"(?mi)^##\s+(?:Notes\s+&\s+Discoveries|Learnings)\b(.*?)(?=(?:^##\s+)|\Z)", re.DOTALL | re.MULTILINE
    )
...
def main() -> None:
    ...
    parser_cons = subparsers.add_parser(
        "consolidate", help="Consolidate task notes and discoveries into extracted_learnings.md"
    )
