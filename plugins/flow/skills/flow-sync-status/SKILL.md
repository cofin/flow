...
1. **Direct Frontmatter Sync (`/flow:sync`)**: Inspect each `.agents/bundles/specs/<flow_id>/tasks/<short_id>.md`, map frontmatter `state` to checklist markers (`[ ]`, `[~]`, `[x] [<sha>]`, `[!]`, `[-]`), and update `spec.md`.
