...
How to apply structured critical reassessment to a claim, assumption, or proposal.
...
Do not reason from memory when you can verify:

- If the claim is about code: read the code
- If the claim is about an API: check the documentation
- If the claim is about performance: look for evidence
- If the claim is about best practices: check if the practice applies to this context
...
Just do the analysis and present the findings directly. The user knows they asked for a challenge — they don't need narration.
