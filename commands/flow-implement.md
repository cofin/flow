...
4. **Claim**: Update the selected task's frontmatter: set `status: in_progress` and `updated_at` to the current ISO-8601 timestamp.
...
6. **Close Task**: Update the task's frontmatter: set `status: closed`, `commit: <sha>`, and `updated_at` to the current ISO-8601 timestamp.
