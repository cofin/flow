...
|---|---|---|---|---|---|
| **Antigravity** | first-class | `plugin.json` | root `hooks.json` with `ANTIGRAVITY_PLUGIN_ROOT` / `AGY_PLUGIN_ROOT` / `PLUGIN_ROOT` root resolution | `agents/*.md` | `/flow:*` |
...
| **OpenCode** | compatible bundle | `.opencode/plugins/flow.js`, `.opencode/agents/*.md`, Agent Skills discovery | no static SessionStart hook | `.opencode/agents/*.md` | `/flow:*` project command files |
